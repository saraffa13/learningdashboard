  ...
  return (
    <div className={`bg-white rounded-lg shadow-md overflow-hidden duration-300 hover:shadow-lg ${
      ...
      <div className="p-4">
        ...
        <h3 className="text-xl font-bold mb-2 line-clamp-2 text-gray-800">{item.title}</h3>
        <p className="text-gray-600 mb-4 line-clamp-3">{item.description}</p>
        <div className="flex flex-wrap gap-2">
          {item.tags.map(tag => (
            <span
              key={tag}
              className="bg-purple-100 text-purple-800 text-xs px-2 py-1 rounded hover:bg-purple-200 transition-colors"
            >
              {tag}
            </span>
          ))}
